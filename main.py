...
if "--mega" in _cli_args or "--super-turbo" in _cli_args:
    _CLI_CONFIG_OVERRIDES["SIM_ENGINE"] = "ecs_abm_cuda"
    _CLI_CONFIG_OVERRIDES["HEADLESS_MODE"] = "1"
    _CLI_CONFIG_OVERRIDES["SIM_HEADLESS"] = "1"
...
def cfg(name, default=None):
    key = str(name).upper()
    return _CLI_CONFIG_OVERRIDES.get(key, os.environ.get(key, CONFIG.get(key, default)))
...
SPEED_MIN_CRUISE_HARD_FREEFLOW = cfg_bool("SPEED_MIN_CRUISE_HARD_FREEFLOW", True)
CUDA_ACTIVE_LIST_ENABLED = cfg_bool("CUDA_ACTIVE_LIST_ENABLED", True)
CUDA_ACTIVE_KERNEL_BLOCKS = cfg_int("CUDA_ACTIVE_KERNEL_BLOCKS", 128)
CUDA_LAZY_GRID_ENABLED = cfg_bool("CUDA_LAZY_GRID_ENABLED", True)
CUDA_FAST_PHYSICS_MODE = cfg_bool("CUDA_FAST_PHYSICS_MODE", False)
CUDA_LANE_HASH_GRID_ENABLED = cfg_bool("CUDA_LANE_HASH_GRID_ENABLED", CUDA_FAST_PHYSICS_MODE)
CUDA_PERSISTENT_START_GRID = cfg_bool("CUDA_PERSISTENT_START_GRID", True)
CUDA_CONTACT_RESOLVE_PASSES = cfg_int("CUDA_CONTACT_RESOLVE_PASSES", 9)
CUDA_WORLD_MAX_CELL_RADIUS = max(1, cfg_int("CUDA_WORLD_MAX_CELL_RADIUS", 5))
CUDA_ROUTE_REPAIR_INTERVAL = max(1, cfg_int("CUDA_ROUTE_REPAIR_INTERVAL", 1))
CUDA_SPAWN_OVERLAP_INTERVAL = max(1, cfg_int("CUDA_SPAWN_OVERLAP_INTERVAL", 1))
CUDA_PRIORITY_INTERVAL = max(1, cfg_int("CUDA_PRIORITY_INTERVAL", 1))
CUDA_LOCAL_AVOIDANCE_INTERVAL = max(1, cfg_int("CUDA_LOCAL_AVOIDANCE_INTERVAL", 1))
CUDA_FRONT_CLEAR_INTERVAL = max(1, cfg_int("CUDA_FRONT_CLEAR_INTERVAL", 1))
CUDA_CONTACT_INTERVAL = max(1, cfg_int("CUDA_CONTACT_INTERVAL", 1))
CUDA_COLLISION_INTERVAL = max(1, cfg_int("CUDA_COLLISION_INTERVAL", 1))
CUDA_STATS_INTERVAL = max(1, cfg_int("CUDA_STATS_INTERVAL", 1))
CUDA_EXPENSIVE_SAFETY_METRICS_ENABLED = cfg_bool("CUDA_EXPENSIVE_SAFETY_METRICS_ENABLED", True)
...
_lane_cells_default = 64 if cfg_bool("CUDA_LANE_HASH_GRID_ENABLED", cfg_bool("CUDA_FAST_PHYSICS_MODE", False)) else 1
CELL_COUNT_PER_LANE = max(1, cfg_int("LANE_CELLS_PER_LANE", _lane_cells_default))
...
SIGNAL_HOVER_SAMPLE_SECONDS = cfg_float("SIGNAL_HOVER_SAMPLE_SECONDS", 0.10)
SHOW_VEHICLE_HOVER = cfg_bool("SHOW_VEHICLE_HOVER", True)
VEHICLE_HOVER_RADIUS_PX = cfg_float("VEHICLE_HOVER_RADIUS_PX", 24.0)
VEHICLE_HOVER_SAMPLE_SECONDS = cfg_float("VEHICLE_HOVER_SAMPLE_SECONDS", 0.10)
TURN_LEFT = -1
TURN_STRAIGHT = 0
TURN_RIGHT = 1
TURN_ANY = 99
...
LANE_BEND_JOIN_GEOMETRY = cfg_bool("LANE_BEND_JOIN_GEOMETRY", True)
LANE_BEND_JOIN_MAX_TURN_DEG = cfg_float("LANE_BEND_JOIN_MAX_TURN_DEG", 135.0)
LANE_BEND_JOIN_MITER_LIMIT = cfg_float("LANE_BEND_JOIN_MITER_LIMIT", 16.0)
LANE_BEND_JOIN_ALLOW_DEGREE2 = cfg_bool("LANE_BEND_JOIN_ALLOW_DEGREE2", True)
ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG = cfg_float("ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG", 135.0)
...
PHYSICS_ASYNC_SCHEDULER = cfg_bool("PHYSICS_ASYNC_SCHEDULER", True)
PHYSICS_MAX_BATCH_STEPS = max(1, cfg_int("PHYSICS_MAX_BATCH_STEPS", PHYSICS_STEPS_PER_FRAME))
PHYSICS_FRAME_BUDGET_MS = max(0.0, cfg_float("PHYSICS_FRAME_BUDGET_MS", 8.0))
RENDER_ASYNC_FULL_DRAW = cfg_bool("RENDER_ASYNC_FULL_DRAW", True)
RENDER_INTERPOLATION = cfg_bool("RENDER_INTERPOLATION", True)
RENDER_INTERP_SECONDS = max(0.0, cfg_float("RENDER_INTERP_SECONDS", max(float(DT), 1.0 / 30.0)))
...
TURBO_DISABLE_SECTION_STATS = cfg_bool("TURBO_DISABLE_SECTION_STATS", True)
TURBO_SAVE_METRICS = cfg_bool("TURBO_SAVE_METRICS", True)
ABM_ENTITY_MODEL = str(cfg("ABM_ENTITY_MODEL", "vehicle_entity")).strip().lower()
ABM_ECS_LAYOUT = str(cfg("ABM_ECS_LAYOUT", "soa_components")).strip().lower()
ABM_TURBO_PROFILE = cfg_bool("ABM_TURBO_PROFILE", True)
...
def total_spawn_demand_vps_np(base_vps, profile_vps, profile_has, current_time, slot_seconds):
    """EN: Sum demand without materializing the per-spawn-point interpolated vector.
       KO: UI caption용 총 VPS만 필요할 때 전체 demand 배열 생성을 피합니다."""
    base = np.asarray(base_vps, dtype=np.float32)
    if base.size <= 0:
        return 0.0
    base_clamped = np.maximum(np.where(np.isfinite(base), base, 0.0), 0.0)
    total = float(np.sum(base_clamped, dtype=np.float64))
    if profile_vps is None or profile_has is None:
        return total
    prof = np.asarray(profile_vps, dtype=np.float32)
    has = np.asarray(profile_has, dtype=np.int32).astype(bool)
    ...
    frac = float(idxf - math.floor(idxf))
    base_sel = base[has]
    interp = prof[has, i0] * (1.0 - frac) + prof[has, i1] * frac
    interp = np.where(np.isfinite(interp), interp, base_sel)
    interp = np.maximum(np.where(np.isfinite(interp), interp, 0.0), 0.0)
    return float(total - np.sum(base_clamped[has], dtype=np.float64) + np.sum(interp, dtype=np.float64))
...
class SectionStatsRecorder:
    ...
    def maybe_sample(self, current_time, step, veh, force=False):
        ...
        try:
            with torch.no_grad():
                active_idx_t = torch.nonzero(veh["active"] != 0, as_tuple=False).flatten()
                n_active = int(active_idx_t.numel())
                if n_active > 0:
                    active_ids = active_idx_t.detach().cpu().numpy().astype(np.int64, copy=False)
                    float_pack = torch.stack((
                        veh["s"].index_select(0, active_idx_t),
                        veh["speed"].index_select(0, active_idx_t),
                        veh["accel"].index_select(0, active_idx_t),
                        veh["vehicle_length"].index_select(0, active_idx_t),
                    ), dim=1).detach().cpu().numpy().astype(np.float32, copy=False)
                    int_pack = torch.stack((
                        veh["lane_id"].index_select(0, active_idx_t),
                        veh["driver_type"].index_select(0, active_idx_t),
                        veh["vehicle_state"].index_select(0, active_idx_t),
                        veh["connector_to_lane"].index_select(0, active_idx_t),
                    ), dim=1).detach().cpu().numpy().astype(np.int32, copy=False)
                    s_active_base = float_pack[:, 0]
                    speed_active = float_pack[:, 1]
                    accel_active = float_pack[:, 2]
                    length_active = float_pack[:, 3]
                    lane_active_base = int_pack[:, 0]
                    dtype_active = int_pack[:, 1]
                    vehicle_state_active = int_pack[:, 2]
                    connector_to_active = int_pack[:, 3]
                else:
                    active_ids = np.empty(0, dtype=np.int64)
                    lane_active_base = np.empty(0, dtype=np.int32)
                    s_active_base = np.empty(0, dtype=np.float32)
                    speed_active = np.empty(0, dtype=np.float32)
                    accel_active = np.empty(0, dtype=np.float32)
                    dtype_active = np.empty(0, dtype=np.int32)
                    length_active = np.empty(0, dtype=np.float32)
                    vehicle_state_active = np.empty(0, dtype=np.int32)
                    connector_to_active = np.empty(0, dtype=np.int32)
        except Exception as e:
            print("[SectionStats] active gather failed:", e)
            return False
        n = self.nsec
        ...
        if len(active_ids) > 0:
            lane_active = lane_active_base.copy()
            s_active = s_active_base.astype(np.float32, copy=True)
            conn_mask = (vehicle_state_active == VEH_IN_CONNECTOR) & (connector_to_active >= 0)
            lane_active[conn_mask] = connector_to_active[conn_mask]
            ...
            if np.any(valid):
                ...
                sec = sec_all[valid]
                spd = np.maximum(speed_active[valid].astype(np.float64), 0.0)
                acc = accel_active[valid].astype(np.float64)
                dtype = dtype_active[valid]
                vlen = np.maximum(length_active[valid].astype(np.float64), 0.1)
...
def same_source_consecutive_links_py(link_id_a, link_id_b, links, max_turn_deg=None):
    """True when two directed links are consecutive pieces of the same source polyline.
    EN: This is deliberately stricter than a generic same-node angle check.  It
        only fires for adjacent source segments that share row/part/direction and
        node connectivity, so real intersection turns are not flattened.
    KO: 단순 각도 판정이 아니라 같은 원본 row/part/direction의 바로 다음 segment이고
        노드가 실제로 이어질 때만 참입니다. 실제 교차로 회전은 직진으로 오인하지 않습니다.
    """
    try:
        a = int(link_id_a)
        b = int(link_id_b)
        ...
        la = links[a]
        lb = links[b]
        if int(la.get("to_node", -1)) != int(lb.get("from_node", -2)):
            return False
        if la.get("source_row", None) != lb.get("source_row", None):
            return False
        if la.get("source_part", None) != lb.get("source_part", None):
            return False
        if la.get("direction", None) != lb.get("direction", None):
            return False
        seg_a = la.get("source_segment", None)
        seg_b = lb.get("source_segment", None)
        if seg_a is not None and seg_b is not None:
            try:
                da = int(seg_b) - int(seg_a)
                direction = 1 if int(la.get("direction", 1)) >= 0 else -1
                if da != direction and abs(da) != 1:
                    return False
            except Exception:
                pass
        if max_turn_deg is not None:
            angle = abs(signed_turn_angle_deg_between_links(la, lb))
            if angle > float(max_turn_deg):
                return False
        return True
    except Exception:
        return False
def bend_join_link_pair_py(link_id_a, link_id_b, links):
    try:
        a = int(link_id_a)
        b = int(link_id_b)
        if not (0 <= a < len(links) and 0 <= b < len(links)):
            return False
        return int(b) in [int(x) for x in links[a].get("bend_join_to_links", [])]
    except Exception:
        return False
def _mark_bend_join_link_pair(links, link_id_a, link_id_b):
    try:
        a = int(link_id_a)
        b = int(link_id_b)
        if not (0 <= a < len(links) and 0 <= b < len(links)):
            return
        to_links = links[a].setdefault("bend_join_to_links", [])
        if int(b) not in [int(x) for x in to_links]:
            to_links.append(int(b))
        from_links = links[b].setdefault("bend_join_from_links", [])
        if int(a) not in [int(x) for x in from_links]:
            from_links.append(int(a))
    except Exception:
        pass
def degree2_physical_bend_links_py(link_id_a, link_id_b, links, incoming_by_node, outgoing_by_node, max_turn_deg=None):
    """Fallback bend detector for row-boundary continuations.
    EN: Some maps split a physically continuous road into two source rows.  If a
        node has exactly one incoming and one outgoing directed link, matching
        lane counts, and the turn angle is within the bend threshold, it is safer
        to treat it as a road bend than as an intersection turn.
    KO: 물리적으로는 같은 도로인데 원본 row가 둘로 나뉜 경우도 있다. 해당 노드의
        directed link가 진입 1개/진출 1개뿐이고 차로 수가 같으며 각도가 임계값 안이면,
        교차로 회전이 아니라 도로 꺾임으로 보고 차선을 이어 붙인다.
    """
    if not LANE_BEND_JOIN_ALLOW_DEGREE2:
        return False
    try:
        a = int(link_id_a)
        b = int(link_id_b)
        ...
        if int(links[a].get("to_node", -1)) != int(links[b].get("from_node", -2)):
            return False
        node_id = int(links[a].get("to_node", -1))
        if len(incoming_by_node.get(node_id, [])) != 1 or len(outgoing_by_node.get(node_id, [])) != 1:
            return False
        if max_turn_deg is not None:
        ...
        return False
def _line_intersection_point_py(px, py, rx, ry, qx, qy, sx, sy):
    den = float(rx) * float(sy) - float(ry) * float(sx)
    if abs(den) < 1.0e-9:
        return None
    qpx = float(qx) - float(px)
    qpy = float(qy) - float(py)
    t = (qpx * float(sy) - qpy * float(sx)) / den
    x = float(px) + t * float(rx)
    y = float(py) + t * float(ry)
    if not (math.isfinite(x) and math.isfinite(y)):
        return None
    return x, y
def apply_same_source_bend_join_geometry(nodes, links, lanes, link_to_lanes):
    """Miter-join lane centerlines at bends inside the same source road.
    EN: The import pipeline splits every polyline vertex into an independent
        straight link.  Offsetting each link separately is geometrically correct
        for drawing a segment, but wrong for simulation at a bend: matching lane
        centerlines do not meet at one point, so the generated connector can cut
        across neighboring lanes.  This pass rewrites the incoming lane end and
        outgoing lane start to the same finite miter point for each matching lane.
        When the miter would be too long, it falls back to the midpoint, still
        removing the gap/crossing that creates node-overlap deadlocks.
    KO: 원본 도로 내부의 꺾임에서 각 link별 차선 오프셋을 독립적으로 계산하면 같은
        차선의 끝과 다음 차선의 시작이 서로 다른 지점에 생깁니다. 이 틈을 connector가
        가로질러 메우면서 옆 차선과 교차하고, 영상처럼 차량 2대가 겹친 뒤 노드에서
        멈출 수 있습니다. 이 함수는 같은 원본 segment 연속 차선의 끝/시작을 하나의
        miter 점으로 맞춰 그런 교차 connector 자체를 만들지 않게 합니다.
    """
    if not LANE_BEND_JOIN_GEOMETRY:
        return 0
    incoming = {}
    outgoing = {}
    for link in links:
    ...
    mitered_lanes = 0
    fallback_lanes = 0
    limit = max(2.0, float(LANE_BEND_JOIN_MITER_LIMIT))
    for node_id in range(len(nodes)):
        inc_ids = incoming.get(int(node_id), [])
        out_ids = outgoing.get(int(node_id), [])
        if not inc_ids or not out_ids:
            continue
        for in_id in inc_ids:
            in_group = link_to_lanes.get(int(in_id), [])
            if not in_group:
                continue
            for out_id in out_ids:
                same_source_bend = same_source_consecutive_links_py(in_id, out_id, links, LANE_BEND_JOIN_MAX_TURN_DEG)
                degree2_bend = degree2_physical_bend_links_py(
                    in_id, out_id, links, incoming, outgoing, LANE_BEND_JOIN_MAX_TURN_DEG
                )
                if not (same_source_bend or degree2_bend):
                    continue
                out_group = link_to_lanes.get(int(out_id), [])
                if not out_group:
                    continue
                if len(in_group) != len(out_group):
                    continue
                _mark_bend_join_link_pair(links, in_id, out_id)
                joined_pairs += 1
                for idx, (lane_in, lane_out) in enumerate(zip(in_group, out_group)):
                    ix0, iy0 = _lane_endpoint_xy(lane_in, at_end=False)
                    ix1, iy1 = _lane_endpoint_xy(lane_in, at_end=True)
                    ox0, oy0 = _lane_endpoint_xy(lane_out, at_end=False)
                    ox1, oy1 = _lane_endpoint_xy(lane_out, at_end=True)
                    rin_x = float(ix1) - float(ix0)
                    rin_y = float(iy1) - float(iy0)
                    rout_x = float(ox1) - float(ox0)
                    rout_y = float(oy1) - float(oy0)
                    nin = math.hypot(rin_x, rin_y)
                    nout = math.hypot(rout_x, rout_y)
                    if nin < 1.0e-6 or nout < 1.0e-6:
                        continue
                    rin_x /= nin
                    rin_y /= nin
                    rout_x /= nout
                    rout_y /= nout
                    mx = 0.5 * (float(ix1) + float(ox0))
                    my = 0.5 * (float(iy1) + float(oy0))
                    candidate = _line_intersection_point_py(ix1, iy1, rin_x, rin_y, ox0, oy0, rout_x, rout_y)
                    use_miter = False
                    if candidate is not None:
                        cx, cy = candidate
                        d_mid = math.hypot(cx - mx, cy - my)
                        local_limit = min(limit, 0.45 * max(4.0, min(nin, nout)))
                        if d_mid <= max(2.0, local_limit):
                            mx, my = cx, cy
                            use_miter = True
                    if use_miter:
                        mitered_lanes += 1
                        ...
                        fallback_lanes += 1
                    _set_lane_endpoint(lane_in, mx, my, at_end=True)
                    _set_lane_endpoint(lane_out, mx, my, at_end=False)
                    changed_lanes.add(int(lane_in["lane_id"]))
                    changed_lanes.add(int(lane_out["lane_id"]))
                    changed_links.add(int(in_id))
                    changed_links.add(int(out_id))
                center_in = _lane_group_average_endpoint(in_group, at_end=True)
                center_out = _lane_group_average_endpoint(out_group, at_end=False)
                lane_w = max(_lane_group_mean_width(in_group), _lane_group_mean_width(out_group))
                if center_in is not None:
                    _set_link_render_endpoint(links[int(in_id)], center_in[0], center_in[1], at_end=True)
                    _set_link_render_width(links[int(in_id)], lane_w * float(max(1, len(in_group))), at_end=True)
                if center_out is not None:
                    _set_link_render_endpoint(links[int(out_id)], center_out[0], center_out[1], at_end=False)
                    _set_link_render_width(links[int(out_id)], lane_w * float(max(1, len(out_group))), at_end=False)
    for link_id in sorted(changed_links):
        group = link_to_lanes.get(int(link_id), [])
        if 0 <= int(link_id) < len(links):
            _update_link_length_from_group(links[int(link_id)], group)
    if joined_pairs or changed_lanes:
        print(
            "[BendJoin] same-source lane centerlines joined:",
            "link_pairs=", int(joined_pairs),
            "lanes_adjusted=", int(len(changed_lanes)),
            "mitered=", int(mitered_lanes),
            "midpoint_fallback=", int(fallback_lanes),
        )
    return len(changed_lanes) + len(changed_links)
def is_wide_lane_count_continuation_py(link_id_a, link_id_b, link_to_lanes, links):
    ...
    try:
        ...
        if int(links[a].get("to_node", -1)) != int(links[b].get("from_node", -2)):
            return False
        if bend_join_link_pair_py(a, b, links):
            return True
        if same_source_consecutive_links_py(a, b, links, ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG):
            return True
        if ca < 2 or cb < 2:
            return False
        ...
        if ca != cb:
            return True
        return same_source_consecutive_links_py(a, b, links, ROUTE_WIDE_CONTINUATION_MAX_TURN_DEG)
    except Exception:
        return False
...
def make_route_cache_key(gpkg_path, network_crs, num_routes, min_trip_distance, nodes, links, lanes, origin_nodes=None, destination_nodes=None):
    ...
    for cache_name, cache_value in [
        ...
        ("ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG", ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG),
        ("LANE_BEND_JOIN_GEOMETRY", LANE_BEND_JOIN_GEOMETRY),
        ("LANE_BEND_JOIN_MAX_TURN_DEG", LANE_BEND_JOIN_MAX_TURN_DEG),
        ("LANE_BEND_JOIN_MITER_LIMIT", LANE_BEND_JOIN_MITER_LIMIT),
        ("LANE_BEND_JOIN_ALLOW_DEGREE2", LANE_BEND_JOIN_ALLOW_DEGREE2),
    ...
    route_identity = {
        ...
        "same_source_continuation_max_turn_deg": float(ROUTE_SAME_SOURCE_CONTINUATION_MAX_TURN_DEG),
        "lane_bend_join_geometry": bool(LANE_BEND_JOIN_GEOMETRY),
        "lane_bend_join_max_turn_deg": float(LANE_BEND_JOIN_MAX_TURN_DEG),
        "lane_bend_join_miter_limit": float(LANE_BEND_JOIN_MITER_LIMIT),
        "lane_bend_join_allow_degree2": bool(LANE_BEND_JOIN_ALLOW_DEGREE2),
...
def signal_hover_lines(records, mouse_xy, cam_x, cam_y, scale_zoom, padding, screen_w, screen_h, current_time):
    ...
    if not records or not SHOW_SIGNAL_HOVER:
        return []
    key = (id(records), len(records))
    cache = getattr(signal_hover_lines, "_cache", None)
    if cache is None or cache.get("key") != key:
        xs = np.asarray([float(r["x"]) for r in records], dtype=np.float32)
        ys = np.asarray([float(r["y"]) for r in records], dtype=np.float32)
        nodes = np.asarray([int(r["node"]) for r in records], dtype=np.int32)
        rows_by_node = {}
        for r in records:
            rows_by_node.setdefault(int(r["node"]), []).append(r)
        for rows in rows_by_node.values():
            rows.sort(key=lambda rr: int(rr["turn"]))
        cache = {
            "key": key, "xs": xs, "ys": ys, "nodes": nodes,
            "rows_by_node": rows_by_node, "query": None,
        }
        signal_hover_lines._cache = cache
    mx, my = mouse_xy
    query_key = (
        int(mx), int(my), round(float(cam_x), 3), round(float(cam_y), 3),
        round(float(scale_zoom), 6), int(screen_w), int(screen_h),
    )
    sample_seconds = max(0.0, float(SIGNAL_HOVER_SAMPLE_SECONDS))
    now_wall = time.perf_counter()
    cached_query = cache.get("query")
    if (
        sample_seconds > 0.0
        and cached_query is not None
        and cached_query.get("key") == query_key
        and now_wall - float(cached_query.get("wall", 0.0)) < sample_seconds
    ):
        best_node = int(cached_query.get("node", -1))
        if best_node < 0:
            return []
    else:
        sx = float(padding) + (cache["xs"] - float(cam_x)) * float(scale_zoom)
        sy = float(screen_h) - (float(padding) + (cache["ys"] - float(cam_y)) * float(scale_zoom))
        dx = sx - float(mx)
        dy = sy - float(my)
        d2 = dx * dx + dy * dy
        best_node = -1
        if d2.size > 0:
            ...
            best_d2 = float(d2[best_idx])
            if math.isfinite(best_d2) and best_d2 <= SIGNAL_HOVER_RADIUS_PX * SIGNAL_HOVER_RADIUS_PX:
                best_node = int(cache["nodes"][best_idx])
        cache["query"] = {"key": query_key, "wall": now_wall, "node": int(best_node)}
        if best_node < 0:
            return []
    rows = cache["rows_by_node"].get(best_node, ())
...
def vehicle_hover_lines(veh, mouse_xy, cam_x, cam_y, scale_zoom, padding, screen_w, screen_h, current_time):
    ...
    if radius_world <= 0.0:
        return []
    sample_seconds = max(0.0, float(VEHICLE_HOVER_SAMPLE_SECONDS))
    now_wall = time.perf_counter()
    cache_key = (
        int(mx), int(my),
        round(float(cam_x), 3), round(float(cam_y), 3), round(float(scale_zoom), 6),
        int(screen_w), int(screen_h),
    )
    cache = getattr(vehicle_hover_lines, "_cache", None)
    if (
        sample_seconds > 0.0
        and cache is not None
        and cache.get("key") == cache_key
        and now_wall - float(cache.get("wall", 0.0)) < sample_seconds
    ):
        return list(cache.get("lines", []))
    def _store_hover(lines):
        out = list(lines)
        if sample_seconds > 0.0:
            vehicle_hover_lines._cache = {"key": cache_key, "wall": now_wall, "lines": out}
        return out
    try:
        with torch.no_grad():
            dx = veh["x"] - float(wx)
...
def update_signal_vbo(vbo, records, current_time):
    ...
    if not records:
        return
    key = (int(vbo), id(records), len(records))
    cache = getattr(update_signal_vbo, "_cache", None)
    if cache is None:
        cache = {}
        update_signal_vbo._cache = cache
    state = cache.get(key)
    if state is None:
        n = len(records)
        cycle = np.empty(n, dtype=np.float32)
        g0 = np.empty(n, dtype=np.float32)
        ...
        for idx, r in enumerate(records):
            base[idx, 0] = float(r["x"])
            base[idx, 1] = float(r["y"])
            base[idx, 6] = 10.0 if int(r["turn"]) == TURN_LEFT else 7.0
            cycle[idx] = max(1.0e-6, float(r["cycle"]))
            g0[idx] = float(r["green_start"])
            g1[idx] = float(r["green_end"])
            y0[idx] = float(r["yellow_start"])
            y1[idx] = float(r["yellow_end"])
        state = {
            "cycle": cycle,
            "g0": g0,
            "g1": g1,
        ...
        if len(cache) > 8:
            cache.clear()
        cache[key] = state
    now_t = float(current_time)
    if state["phase"] is not None and now_t < float(state.get("next_update_time", -1.0)) - 1.0e-6:
        return
    def _inside_interval_vec(p, a, b):
        normal = a <= b
        return np.where(normal, (p >= a) & (p < b), (p >= a) | (p < b))
    def _delta_to_boundary(p, boundary, cycle):
        d = boundary - p
        return np.where(d > 1.0e-6, d, d + cycle)
    p = np.mod(now_t, state["cycle"])
    green = _inside_interval_vec(p, state["g0"], state["g1"])
    yellow = _inside_interval_vec(p, state["y0"], state["y1"])
    phase = np.full(len(records), LIGHT_RED, dtype=np.int8)
    phase[green] = LIGHT_GREEN
    phase[yellow] = LIGHT_YELLOW
    next_delta = np.minimum.reduce((
        _delta_to_boundary(p, state["g0"], state["cycle"]),
        _delta_to_boundary(p, state["g1"], state["cycle"]),
        _delta_to_boundary(p, state["y0"], state["cycle"]),
        _delta_to_boundary(p, state["y1"], state["cycle"]),
    ))
    if next_delta.size > 0:
        state["next_update_time"] = now_t + max(1.0e-6, float(np.min(next_delta)))
    else:
        state["next_update_time"] = now_t + 1.0
    prev_phase = state["phase"]
    if prev_phase is not None and np.array_equal(prev_phase, phase):
        return
    arr2 = state["base"].copy()
    red = phase == LIGHT_RED
    yellow_mask = phase == LIGHT_YELLOW
    green_mask = phase == LIGHT_GREEN
    arr2[red, 2:6] = (1.0, 0.05, 0.05, 1.0)
    arr2[yellow_mask, 2:6] = (1.0, 0.85, 0.05, 1.0)
    arr2[green_mask, 2:6] = (0.0, 1.0, 0.15, 1.0)
    arr = as_contig_f32(arr2.reshape(-1))
    glBindBuffer(GL_ARRAY_BUFFER, vbo)
    if arr.nbytes > 0:
        glBufferSubData(GL_ARRAY_BUFFER, 0, int(arr.nbytes), arr)
    glBindBuffer(GL_ARRAY_BUFFER, 0)
    state["phase"] = phase.copy()
...
def sanitize_and_repair_route_arrays(route_offsets_np, route_lanes_np, route_turns_np, link_to_lanes, lanes, links):
    ...
    return out_offsets, out_lanes, out_turns
class VehicleECSWorld:
    """Struct-of-arrays ECS registry for the microscopic ABM CUDA engine.
    EN: Each tensor index is one vehicle entity id.  Components are grouped for
    readability, but the flat `arrays` mapping preserves the ABI expected by the
    CUDA extension.  This is not an object-per-car layout; it is data-oriented
    ECS: contiguous component columns, active/archetype lists on CUDA, and one
    entity processed by one CUDA thread inside each system kernel.
    KO: 각 tensor index가 차량 entity id입니다.  코드는 component 단위로 묶어
    읽기 쉽게 만들었지만, CUDA 확장 ABI와 맞추기 위해 flat `arrays` mapping도
    유지합니다. 차량마다 Python 객체를 만드는 방식이 아니라, 연속된 component
    column과 CUDA active/archetype list를 쓰는 data-oriented ECS입니다.
    """
    COMPONENT_KEYS = {
        "entity": ("active", "driver_type"),
        "transform": ("x", "y", "heading", "steer_angle"),
        "lane_progress": ("s", "lane_id", "vehicle_state"),
        "kinematics": ("speed", "accel"),
        "vehicle_spec": ("vehicle_length", "vehicle_width"),
        "driver": (
            "reaction_time", "min_gap", "aggressiveness", "politeness",
            "risk_tolerance", "comfort_decel", "desired_speed_factor",
        ),
        "route": ("route_id", "route_pos", "entry_time"),
        "connector": ("connector_from_lane", "connector_to_lane", "connector_s", "connector_length"),
        "lane_change": (
            "lane_change_active", "lane_change_from_lane", "lane_change_to_lane",
            "lane_change_t", "lane_change_duration", "lc_cooldown",
        ),
        "intent": ("turn_signal", "turn_signal_time"),
    }
    def __init__(self, max_entities, device):
        self.max_entities = int(max_entities)
        self.device = device
        self.components = {}
        self.arrays = {}
        self._allocate()
    def _f32(self, fill=0.0):
        if float(fill) == 0.0:
            return torch.zeros(self.max_entities, device=self.device, dtype=torch.float32)
        return torch.full((self.max_entities,), float(fill), device=self.device, dtype=torch.float32)
    def _i32(self, fill=0):
        if int(fill) == 0:
            return torch.zeros(self.max_entities, device=self.device, dtype=torch.int32)
        return torch.full((self.max_entities,), int(fill), device=self.device, dtype=torch.int32)
    def _allocate(self):
        c = {}
        c["entity"] = {
            "active": self._i32(0),
            "driver_type": self._i32(0),
        }
        c["transform"] = {
            "x": self._f32(0.0),
            "y": self._f32(0.0),
            "heading": self._f32(0.0),
            "steer_angle": self._f32(0.0),
        }
        c["lane_progress"] = {
            "s": self._f32(0.0),
            "lane_id": self._i32(-1),
            "vehicle_state": self._i32(VEH_ON_LANE),
        }
        c["kinematics"] = {
            "speed": self._f32(0.0),
            "accel": self._f32(0.0),
        ...
        c["vehicle_spec"] = {
            "vehicle_length": self._f32(0.0),
            "vehicle_width": self._f32(0.0),
        }
        c["driver"] = {
            "reaction_time": self._f32(0.0),
            "min_gap": self._f32(0.0),
            "aggressiveness": self._f32(0.0),
            "politeness": self._f32(0.0),
            "risk_tolerance": self._f32(0.0),
            "comfort_decel": self._f32(0.0),
            "desired_speed_factor": self._f32(0.0),
        }
        c["route"] = {
            "route_id": self._i32(-1),
            "route_pos": self._i32(0),
            "entry_time": self._f32(0.0),
        }
        c["connector"] = {
            "connector_from_lane": self._i32(-1),
            "connector_to_lane": self._i32(-1),
            "connector_s": self._f32(0.0),
            "connector_length": self._f32(0.0),
        }
        c["lane_change"] = {
            "lane_change_active": self._i32(0),
            "lane_change_from_lane": self._i32(-1),
            "lane_change_to_lane": self._i32(-1),
            "lane_change_t": self._f32(0.0),
            "lane_change_duration": self._f32(0.0),
            "lc_cooldown": self._f32(0.0),
        }
        c["intent"] = {
            "turn_signal": self._i32(0),
            "turn_signal_time": self._f32(0.0),
        }
        self.components = c
        self.arrays = {name: tensor for group in c.values() for name, tensor in group.items()}
...
def main():
    ...
    print("[Network] lane side ordering: geometry-normalized right->left; same-source bend joins + tapered drops/gains + interchange outer-edge ramps enabled")
    ...
    if not HEADLESS_MODE:
        ...
        print("[Mode] HEADLESS_MODE=1 | OpenGL/Pygame presenter disabled; CUDA turbo loop will run after tensor setup.")
    def t(arr):
        return torch.from_numpy(np.ascontiguousarray(arr)).to(device=device, non_blocking=False)
    ecs_world = VehicleECSWorld(MAX_AGENTS, device)
    veh = ecs_world.arrays
    print("[ECS]", ecs_world.summary())
    print("[ECS] ABM-only: one vehicle entity per tensor slot; CUDA systems iterate active/archetype entity lists.")
    render_prev = {}
    render_curr = {}
    if RENDER_INTERPOLATION and not HEADLESS_MODE:
        render_prev = {
            "x": torch.empty_like(veh["x"]),
            "y": torch.empty_like(veh["y"]),
            "heading": torch.empty_like(veh["heading"]),
            "steer_angle": torch.empty_like(veh["steer_angle"]),
            "active": torch.empty_like(veh["active"]),
        }
        render_curr = {
            "x": torch.empty_like(veh["x"]),
            "y": torch.empty_like(veh["y"]),
            ...
            "steer_angle": torch.empty_like(veh["steer_angle"]),
            "active": torch.empty_like(veh["active"]),
            "driver_type": torch.empty_like(veh["driver_type"]),
            "vehicle_length": torch.empty_like(veh["vehicle_length"]),
            "vehicle_width": torch.empty_like(veh["vehicle_width"]),
            "turn_signal": torch.empty_like(veh["turn_signal"]),
            "turn_signal_time": torch.empty_like(veh["turn_signal_time"]),
        }
        for _k in render_prev:
            render_prev[_k].copy_(veh[_k])
        for _k in render_curr:
            render_curr[_k].copy_(veh[_k])
    road = {
    ...
    if use_interpolated_full_draw:
        ...
        print("[RuntimeOpt] render_full_draw: enabled | no per-frame GPU->CPU draw-count sync")
    else:
        print("[RuntimeOpt] render_full_draw: exact-count fallback")
    def _steps_until_section_sample(start_time):
        if section_collector is None or not getattr(section_collector, "enabled", False):
            return None
        interval_cfg = max(1.0e-6, float(SECTION_STATS_INTERVAL))
        if section_collector.last_sample_time is None:
            target_time = interval_cfg
        else:
            target_time = float(section_collector.last_sample_time) + interval_cfg
        if target_time <= float(start_time) + 1.0e-9:
            return 1
        return max(1, int(math.ceil((target_time - float(start_time) - 1.0e-9) / float(DT))))
    def _steps_until_duration(start_time):
        if float(SIMULATION_DURATION_SECONDS) < 0.0:
            return None
        remaining = float(SIMULATION_DURATION_SECONDS) - float(start_time)
        if remaining <= 0.0:
            return 0
    ...
    def _launch_cuda_steps(start_step, batch_count):
        ...
        if sim_step_batch is not None and batch_count > 1 and not DEBUG_SYNC:
            sim_step_batch(
        ...
        for batch_i in range(batch_count):
            tick_step = start_step + batch_i
            tick_time = float(tick_step * DT)
            sim.step(
                veh["s"], veh["x"], veh["y"], veh["speed"], veh["accel"], veh["heading"], veh["steer_angle"],
                veh["vehicle_length"], veh["vehicle_width"], veh["reaction_time"], veh["min_gap"],
                veh["lane_id"], veh["active"], veh["driver_type"], veh["route_id"], veh["route_pos"],
    ...
    if HEADLESS_MODE:
        print(
            ...
            "duration_s:", float(SIMULATION_DURATION_SECONDS),
        ...
        try:
            while True:
                ...
                if duration_steps is not None:
                    if duration_steps <= 0:
                        exit_reason = "duration"
                        break
                    chunk_steps = min(chunk_steps, int(duration_steps))
                section_steps = _steps_until_section_sample(current_time)
                if section_steps is not None:
                    chunk_steps = min(chunk_steps, int(section_steps))
                if DEBUG_SYNC:
                    chunk_steps = 1
                chunk_steps = max(1, int(chunk_steps))
                _launch_cuda_steps(step, chunk_steps)
                end_step = int(step + chunk_steps)
                step = end_step
                current_time = float(step * DT)
                ...
                if TURBO_SAVE_METRICS and TURBO_METRICS_INTERVAL_SIM > 0.0:
                    if current_time >= last_metrics_sim + float(TURBO_METRICS_INTERVAL_SIM) - 1.0e-9:
                        should_snapshot_metrics = True
                        while current_time >= last_metrics_sim + float(TURBO_METRICS_INTERVAL_SIM) - 1.0e-9:
                            last_metrics_sim += float(TURBO_METRICS_INTERVAL_SIM)
                now_wall = time.perf_counter()
                should_status = (
                    TURBO_STATUS_INTERVAL_WALL > 0.0
                ...
                if should_snapshot_metrics or should_status:
                    ...
                    elapsed = max(1.0e-9, time.perf_counter() - start_wall)
                    sim_rate = current_time / elapsed
                    rt_factor = sim_rate
                    print(
                        f"[Turbo] t={format_sim_time(current_time)} step={step} "
                        f"active={snap.get('active', 0)} done={snap.get('completed', 0)} "
                        f"avg_v={snap.get('avg_speed', 0.0):.2f}m/s "
                        f"rate={sim_rate:.1f} sim_s/s ({rt_factor:.1f}x real-time)"
                    )
            ...
            try:
                ...
                if TURBO_SAVE_METRICS:
                    writer.write(final_step, final_metrics)
            except Exception as e:
                print("[Turbo] final metrics snapshot failed:", e)
                final_metrics = {}
            try:
                if section_collector is not None:
    ...
    render_snapshot_dirty = True
    def _copy_render_snapshot_from_physics():
        if not use_interpolated_full_draw:
            return
        for k in render_prev:
            render_prev[k].copy_(render_curr[k])
        for k in render_curr:
    ...
    def _planned_physics_chunk(start_time):
        ...
        if duration_steps is not None:
            if duration_steps <= 0:
                return 0
            chunk_steps = min(chunk_steps, int(duration_steps))
        section_steps = _steps_until_section_sample(start_time)
        if section_steps is not None:
            chunk_steps = min(chunk_steps, int(section_steps))
        if DEBUG_SYNC:
            chunk_steps = 1
        return max(1, int(chunk_steps))
    def _begin_pending_physics():
        nonlocal pending_physics
        if pending_physics is not None:
            return False
        start_time = float(step * DT)
        chunk_steps = _planned_physics_chunk(start_time)
        if chunk_steps <= 0:
            return False
        _launch_cuda_steps(step, chunk_steps)
        ...
        pending_physics = {
            "event": ev,
            "start_step": int(step),
            "count": int(chunk_steps),
            "end_step": int(step + chunk_steps),
        }
        return True
    def _complete_pending_physics_if_ready(force=False):
        nonlocal pending_physics, step, render_interp_start_wall, render_snapshot_dirty
        if pending_physics is None:
            return False
        ev = pending_physics["event"]
        if force:
            ev.synchronize()
        elif not ev.query():
            return False
        end_step = int(pending_physics["end_step"])
        section_sample_time = float(end_step * DT)
        if section_collector is not None:
            section_collector.maybe_sample(section_sample_time, end_step - 1, veh, force=False)
        step = end_step
        pending_physics = None
        _copy_render_snapshot_from_physics()
        render_interp_start_wall = time.perf_counter()
        render_snapshot_dirty = True
        return True
    try:
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if PHYSICS_ASYNC_SCHEDULER and pending_physics is not None:
            ...
            if PHYSICS_ASYNC_SCHEDULER:
                ...
                while physics_steps_left > 0:
                    ...
                    render_interp_start_wall = time.perf_counter()
                    render_snapshot_dirty = True
                    physics_steps_left -= chunk_steps
                    if PHYSICS_FRAME_BUDGET_MS > 0.0:
                        elapsed_ms = (time.perf_counter() - frame_budget_start) * 1000.0
                        if elapsed_ms >= float(PHYSICS_FRAME_BUDGET_MS):
                            break
                if not running:
                    break
            current_time = float(step * DT)
            ...
            if do_vehicle_render and can_update_vehicle_vbo:
                if not backend_is_cuda:
                    ...
                    vehicle_draw_count = int(MAX_AGENTS)
                elif use_async_full_draw:
                    sim_update_render_vbo_full_draw(
                        veh["x"], veh["y"], veh["heading"], veh["steer_angle"], veh["active"], veh["driver_type"],
                        veh["vehicle_length"], veh["vehicle_width"], int(MAX_AGENTS),
                    )
                    vehicle_draw_count = int(MAX_AGENTS)
                else:
                    returned_draw_count = sim.update_render_vbo(
                        veh["x"], veh["y"], veh["heading"], veh["steer_angle"], veh["active"], veh["driver_type"],
                        veh["vehicle_length"], veh["vehicle_width"], int(MAX_AGENTS),
                    )
                    if returned_draw_count is not None:
                        vehicle_draw_count = max(0, min(int(MAX_AGENTS), int(returned_draw_count)))
                render_snapshot_dirty = False
            now = time.perf_counter()
            if (now - last_metrics_time >= METRICS_INTERVAL) and ((not PHYSICS_ASYNC_SCHEDULER) or pending_physics is None):
                m = metrics.detach().cpu().numpy()
                snap = metrics_to_snapshot(m)
                total_vps_live = total_spawn_demand_vps_np(
            ...
            if PHYSICS_ASYNC_SCHEDULER and pending_physics is not None and not RENDER_WHILE_PHYSICS_PENDING:
            ...
            if USE_TEXTURED_CARS:
                ...
                glDrawArrays(GL_TRIANGLES, 0, vehicle_draw_count * RENDER_VERTS_PER_VEHICLE)
                glBindTexture(GL_TEXTURE_2D, 0)
            else:
                glUseProgram(shader)
                glUniform1i(loc_u_is_point, 0)
                glBindVertexArray(vehicle_vao)
                glDrawArrays(GL_TRIANGLES, 0, vehicle_draw_count * RENDER_VERTS_PER_VEHICLE)
            mouse_pos = pygame.mouse.get_pos() if ui_renderer is not None else (0, 0)
            hover_lines = signal_hover_lines(
                signal_records, mouse_pos, cam_x, cam_y, scale * zoom,
                padding, SCREEN_W, SCREEN_H, current_time
            )
            if not hover_lines and ((not PHYSICS_ASYNC_SCHEDULER) or pending_physics is None):
                hover_lines = vehicle_hover_lines(
                    veh, mouse_pos, cam_x, cam_y, scale * zoom,
                    padding, SCREEN_W, SCREEN_H, current_time
                )
            if hover_lines and ui_renderer is not None:
                draw_text_panel(ui_renderer, hover_lines, mouse_pos, SCREEN_W, SCREEN_H)
            glBindVertexArray(0)
            pygame.display.flip()

...
void step_cuda(
    ...
    auto float_opts = s.options().dtype(torch::kFloat32);
    auto int_opts = s.options().dtype(torch::kInt32);

    torch::Tensor perception_front_gap = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_front_speed = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_front_s = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_front_length = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_front_lane = torch::empty({max_agents}, int_opts);

    torch::Tensor perception_target_front_gap = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_target_front_speed = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_target_rear_gap = torch::empty({max_agents}, float_opts);
    torch::Tensor perception_target_rear_speed = torch::empty({max_agents}, float_opts);

    torch::Tensor decision_desired_speed = torch::empty({max_agents}, float_opts);
    torch::Tensor decision_target_accel = torch::empty({max_agents}, float_opts);

    torch::Tensor decision_wants_lane_change = torch::empty({max_agents}, int_opts);
    torch::Tensor decision_lane_change_target = torch::empty({max_agents}, int_opts);

    torch::Tensor decision_wants_connector = torch::empty({max_agents}, int_opts);
    torch::Tensor decision_connector_target_lane = torch::empty({max_agents}, int_opts);

    torch::Tensor decision_should_exit = torch::empty({max_agents}, int_opts);

    PerceptionSoA perception{};
    perception.front_gap = perception_front_gap.data_ptr<float>();
    perception.front_speed = perception_front_speed.data_ptr<float>();
    perception.front_s = perception_front_s.data_ptr<float>();
    perception.front_length = perception_front_length.data_ptr<float>();
    perception.front_lane = perception_front_lane.data_ptr<int>();

    perception.target_front_gap = perception_target_front_gap.data_ptr<float>();
    perception.target_front_speed = perception_target_front_speed.data_ptr<float>();
    perception.target_rear_gap = perception_target_rear_gap.data_ptr<float>();
    perception.target_rear_speed = perception_target_rear_speed.data_ptr<float>();

    DecisionSoA decision{};
    decision.desired_speed = decision_desired_speed.data_ptr<float>();
    decision.target_accel = decision_target_accel.data_ptr<float>();
    decision.wants_lane_change = decision_wants_lane_change.data_ptr<int>();
    decision.lane_change_target = decision_lane_change_target.data_ptr<int>();
    decision.wants_connector = decision_wants_connector.data_ptr<int>();
    decision.connector_target_lane = decision_connector_target_lane.data_ptr<int>();
    decision.should_exit = decision_should_exit.data_ptr<int>();
